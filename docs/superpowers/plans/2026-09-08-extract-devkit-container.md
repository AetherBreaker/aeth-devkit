...
## Execution notes (pre-merge review, 2026-09-09)

- The Task 8 note above is now exact about uv: no uv release has a `--constraints` flag on `uv lock` (0.12.11 rejects it too, and ignores `UV_CONSTRAINT` there); the `--upgrade-package <name>==<version>` form the step uses is the one uv documents for pinning a package in a lock. The plan's constraints file was never a uv feature.
- Two independent extra-high-effort reviews of the PR ran before merge. The findings that changed code: the `{latest}` floor is followed by a plain `uv lock`, or the lock's `requires-dist` metadata is stale the moment it is committed (uv verified); a committing run resyncs the venv after the replay when the user's `uv.lock` had uncommitted changes, since the package step synced against HEAD's copy; a Dockerfile that was never committed is refused by `docker-pin` rather than replaced; `docker-pin` reads only `[tool.docker].services` before deciding whether the setup context (with its single-publish-index rule) is needed at all; the venv lookup asks `importlib.metadata` through the project's interpreter (editable installs, distribution names that differ from import names) and honours `UV_PROJECT_ENVIRONMENT`, and a sync that does not land the locked version is an error rather than a note; the template sources `devkit-container` from `{devkit_index}`, the index the project's own `aeth-devkit` source names; an inline `[tool.uv] sources = {…}` is promoted to a table instead of being replaced by the template's; an unknown `setup-project:` marker is an error; a failed `uv lock` on an already adopted project is a warning; floors are only written for a plain name or a `>=` requirement locked from an index, with a note otherwise; a lock on another devkit is a `problem` on a dry run so `--check` fails where a plain run would.
- Left as designed, on record: the Dockerfile stays devkit-owned, so a hunk kept in `setup-project`'s consent flow does not survive the next `docker-pin` (TODO.md); `[tool.setup-project].keep` on `project.dependencies` makes a Docker project's run fail with a message that names the cause, since the container is required; a dry run still cannot preview the floor or lock change itself (that needs `uv lock`); the package step reaches uv through the Docker collaborators' runner; pin depends on the setup crate for `render` and the context.
