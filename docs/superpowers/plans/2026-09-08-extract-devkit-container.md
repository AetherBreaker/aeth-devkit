...
- Left as designed, on record: the Dockerfile stays devkit-owned, so a hunk kept in `setup-project`'s consent flow does not survive the next `docker-pin` (TODO.md); a dry run still cannot preview the floor or lock change itself (that needs `uv lock`); the package step reaches uv through the Docker collaborators' runner; pin depends on the setup crate for `render` and the context.
