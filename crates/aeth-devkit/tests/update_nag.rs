//! The outdated-version nag reaches stderr from every command. Drives the real `devkit`
//! binary with a fresh cache file so no network is touched; the ordinary command is
//! `docker-pin --dry-run` outside a git repository, which fails at its first check before
//! any network call, and the nag follows the error.
